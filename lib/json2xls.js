...
transform.prepareJson = function(json,config) {
    ...
    if (!(fields instanceof Array)) {
        ...
        captions = Object.keys(fields).map(function(key) {
            return fields[key].caption;
        });
    ...
    res.cols = fields.map(function(key,i) {
        return {
            caption: captions[i] || key,
            type: getType(jsonArr[0] ? jsonArr[0][key] : '', types[i]),
            beforeCellWrite: function(row, cellData, eOpt){
                eOpt.cellType = getType(cellData,types[i]);
                return config.fields[key].beforeCellWrite ? config.fields[key].beforeCellWrite(row, cellData, eOpt) : cellData;
            }
        };
    });
    //rows
    res.rows = jsonArr.map(function(row) {
        return fields.map(function(key) {
            ...
            //stringify objects
            if(value && value.constructor == Object && !config.fields[key].beforeCellWrite) value = JSON.stringify(value);
